...
import net.hwyz.iov.cloud.framework.common.exception.UtilException;
...
public class ExcelUtil<T> {
                ...
                for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet()) {
                    ...
                    if (String.class == fieldType) {
                        ...
                        if (val instanceof String) {
                            val = cn.hutool.core.date.DateUtil.parseDate(val.toString());
    ...
    public void fillExcelData(int index, Row row) {
        ...
        for (int i = startNo; i < endNo; i++) {
            ...
            for (Object[] os : fields) {
                Field field = (Field) os[0];
                Excel excel = (Excel) os[1];
                if (Collection.class.isAssignableFrom(field.getType()) && ObjUtil.isNotNull(subList)) {
    ...
    public void annotationDataStyles(Map<String, CellStyle> styles, Field field, Excel excel) {
        String key = StrUtil.format("data_{}_{}_{}_{}", excel.align(), excel.color(), excel.backgroundColor(), excel.cellType());
    ...
    public Cell createHeadCell(Excel attr, Row row, int column) {
        ...
        if (isSubList()) {
            // 填充默认样式，防止合并单元格样式失效
            sheet.setDefaultColumnStyle(column, styles.get(StrUtil.format("data_{}_{}_{}_{}", attr.align(), attr.color(), attr.backgroundColor(), attr.cellType())));
            if (attr.needMerge()) {
    ...
    public void setCellVo(Object value, Excel attr, Cell cell) {
        if (Excel.ColumnType.STRING == attr.cellType() || Excel.ColumnType.TEXT == attr.cellType()) {
            ...
            cell.setCellValue(ObjUtil.isNull(cellValue) ? attr.defaultValue() : cellValue + attr.suffix());
    ...
    public void setDataValidation(Excel attr, Row row, int column) {
        ...
        if (StrUtil.isNotEmpty(attr.prompt()) || attr.combo().length > 0) {
            if (attr.combo().length > 15 || ArrayUtil.join(attr.combo(), null).length() > 255) {
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column) {
        ...
        try {
            ...
            if (attr.isExport()) {
                ...
                cell.setCellStyle(styles.get(StrUtil.format("data_{}_{}_{}_{}", attr.align(), attr.color(), attr.backgroundColor(), attr.cellType())));
                ...
                if (StrUtil.isNotEmpty(dateFormat) && ObjUtil.isNotNull(value)) {
                    cell.setCellValue(parseDateToStr(dateFormat, value));
                } else if (StrUtil.isNotEmpty(readConverterExp) && ObjUtil.isNotNull(value)) {
    ...
    private Object getValue(Object o, String name) throws Exception {
        if (ObjUtil.isNotNull(o) && StrUtil.isNotEmpty(name)) {
    ...
    public String parseDateToStr(String dateFormat, Object val) {
        ...
        if (val instanceof Date) {
            ...
            str = cn.hutool.core.date.DateUtil.format(((LocalDate) val).atStartOfDay(), dateFormat);
        } else {
            str = val.toString();
        }
        return str;
    ...
    public boolean isSubList() {
        return ObjUtil.isNotNull(subFields) && subFields.size() > 0;
    }
    ...
    public boolean isSubListValue(T vo) {
        return ObjUtil.isNotNull(subFields) && subFields.size() > 0 && ObjUtil.isNotNull(getListCellValue(vo)) && getListCellValue(vo).size() > 0;
    }
